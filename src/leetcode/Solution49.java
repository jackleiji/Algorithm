...
public class Solution49 {
    ...
    public static List<List<String>> groupAnagrams(String[] strs) {
        ...
        int len = strs.length;
        int end_flag = 0;
        boolean[] get_str = new boolean[len];
      ...
      if (flag1) res.add(words1);
        while (end_flag < len){
            if(!get_str[end_flag]){
                List<String> words = new ArrayList<>();
                char[] char_arr = strs[end_flag].toCharArray();
                int[] letter = new int[26];
                //写入字母表
                for(int j = 0;j<char_arr.length;j++){
                    int numb = char_arr[j] - 'a';
                    letter[numb] +=1;
                }
                //单词数量写入字母表
                words.add(strs[end_flag]);
                get_str[end_flag] = true;

                for(int i=end_flag+1;i<len;i++){
                    //不等则下一个
                    if(char_arr.length!= strs[i].length() || get_str[i]){
                        continue;
                    }
                    int[] comp_letter = new int[26];
                    //写入比较字母表
                    for(int j = 0;j<strs[i].length();j++){
                        int numb = strs[i].charAt(j) - 'a';
                        comp_letter[numb] +=1;
                    }
                    boolean flag = false;
                    //比较
                    for(int j=0;j<26;j++){
                        if(comp_letter[j] != letter[j]){
                            flag = true;
                            break;
                        }
                    }
